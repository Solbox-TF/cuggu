...
import { isValidAudioBuffer } from '@/lib/bgm/validation';
 ...
 * POST /api/upload/bgm
 *
 * BGM 오디오 파일 업로드 (단일 파일)
 *
 * FormData:
 * - file: File (MP3/M4A, 최대 10MB)
 * - invitationId: string
 */
export async function POST(request: NextRequest) {
  try {
    ...
    if (!(BGM_CONFIG.ALLOWED_MIME_TYPES as readonly string[]).includes(file.type)) {
      return NextResponse.json(
        { error: `지원하지 않는 형식입니다. MP3 또는 M4A 파일을 업로드해주세요.` },
        { status: 400 }
      );
    }
    ...
    // 8. S3 업로드 (오디오는 원본 그대로, 최적화 없음)
    const { url } = await uploadToS3(buffer, file.type, `${BGM_CONFIG.S3_PREFIX}/${user.id}`);
    ...
    logger.error('BGM upload failed', {
      error: error instanceof Error ? error.message : String(error),
    });

    return NextResponse.json(
      { error: 'BGM 업로드 중 오류가 발생했습니다' },
      { status: 500 }
    );
  }
}
